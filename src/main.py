    ...
    if is_valid:
        ...
        if check:
            game_state.grid = lib.modify_grid(
                player_input, game_state.current_player_turn(), game_state.grid, game_state)
            ...
            if lib.piece_is_powerup(player_input, last_grid_state, game_state.set_of_powerups) == True:
                ...
                lib.powerup_activation_logic(
                    player_input, last_grid_state[player_input[0]][player_input[1]], game_state.n, game_state)
            ...
            if game_state.pieces_placed_on_this_player_turn == game_state.pieces_to_place_for_each_player_turn:
                while True:
                    ...
                    is_valid = lib.validate_powerup_placement(
                        try_coords, game_state.grid, game_state.sub_set_of_replacedable_pieces, game_state.n)
                    ...
                    if is_valid:
                        ...
                        if game_state.pieces_placed_on_this_player_turn == game_state.pieces_to_place_for_each_player_turn:
                            game_state.grid = lib.modify_grid(
                                try_coords, powerup_this_turn, game_state.grid, game_state)
